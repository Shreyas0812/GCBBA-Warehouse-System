...
def main():
    ...
    elif args.config == "gcbba_only":
        # All GCBBA variants (static + dynamic + sensitivity sweep), ss and batch. No baselines.
        configs = [
            c for c in all_configs
            if c["config_name"] in {"static", "dynamic", "static_batch", "dynamic_batch"}
            or c["config_name"].startswith("dynamic_ri")
        ]
    elif args.config == "gcbba_dmchba":
        # GCBBA (all variants, ss + batch) + DMCHBA (ss + batch). No CBBA/SGA.
        configs = [
            c for c in all_configs
            if c["config_name"] in {"static", "dynamic", "static_batch", "dynamic_batch",
                                    "dmchba", "dmchba_batch"}
