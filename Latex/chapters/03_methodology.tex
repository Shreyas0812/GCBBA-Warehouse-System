...
All experiments use a $30 \times 30$ gridworld warehouse, depicted in
Figure~\ref{fig:gridworld}.
The environment contains $\Na = 6$ agents,
$|\mathcal{S}^{\text{in}}| = 8$ induct stations positioned along one wall of
the warehouse, and $|\mathcal{S}^{\text{ej}}| = 38$ eject stations distributed
across the opposite end.
A set of internal obstacles represents shelving aisles, creating a structured
navigation challenge.
Charging stations are located near the centre of the grid.
All agents are initialised at their home positions at $t = 0$.

\begin{figure}[htbp]
  \centering
  % \includegraphics[width=0.6\textwidth]{figures/gridworld}
  \textbf{[FIGURE PLACEHOLDER: Grid world visualisation showing induct stations (blue),
  eject stations (red), obstacles (grey), charging stations (green), and
  agent starting positions.]}
  \caption{The $30 \times 30$ warehouse grid environment used in all
           experiments. Induct stations (blue) are task pickup locations along
           the top wall; eject stations (red) are delivery destinations at the
           bottom. Internal shelving creates constrained navigation corridors.}
  \label{fig:gridworld}
\end{figure}
...
\textbf{Energy model.}
Each agent starts with full energy and consumes one unit per timestep of movement.
When the estimated remaining distance to the nearest charger falls below a
threshold proportional to the agent's current energy, the agent suspends task
execution, travels to the nearest available charging station, charges until
full, and then resumes.
Charging events trigger a reallocation (Section~\ref{sec:method_triggers})
because the agent's effective availability changes.
...
This is especially important in the real-time warehouse setting, where each
allocation call must complete within a per-timestep budget.
...
For the warehouse sizes considered in this thesis ($\Na = 6$), the difference
between CA* and PBS in both path quality and computation time is modest.
...
$w$ timesteps.
This is particularly beneficial in the steady-state setting where missions have
no defined end.
...
Section~\ref{sec:exp_planners} presents a dedicated comparison of all three
planners to characterise the throughput--compute trade-off.
