...
performs which task, and in what order, so as to optimise a system-level
...
The choice between centralised and decentralised architectures
(Sections~\ref{subsec:bg_centralised} and~\ref{subsec:bg_decentralised})
...
Centralised MRTA methods aggregate all information at a single planner that
computes assignments for all the agents.
The classical assignment problem (minimise total cost for a one-to-one
...
for large fleets or real-time settings.
...
$\frac{1}{2}$ of the optimum.

Centralised methods share two practical weaknesses.
...
halts.
These limitations motivate the decentralised alternatives described next.
...
$\frac{1}{2}$-optimality under DMG utilities.

CBBA's key limitation is its \emph{local} consensus protocol: each agent
communicates only with its immediate neighbours, and a single round of exchange
...
which is provably DMG and better aligned with makespan minimisation than the
...
decentralised character of CBBA.
...
Beyond auction-based methods, several other decentralised approaches have been
proposed for MRTA.
\emph{Distributed Constraint Optimisation Problems} (DCOPs) formulate task
allocation as a distributed optimisation over coupled agent variables;
...
convergence time.
\emph{Behaviour-based methods}~\cite{parker1998alliance} allocate tasks
...
The common thread across all existing decentralised methods is the assumption
...
boundary.
This idea is formalised and extended by the rolling horizon approach described
...
mission horizon.
RHCR is one of the six planning strategies evaluated in this thesis
(Section~\ref{subsec:method_rhcr}).

A key design insight from \citet{li2021lifelong} is that optimising for
throughput requires different priority rules than optimising for makespan.
...
Prioritised planning~\cite{silver2005cooperative} decomposes the joint MAPF
...
Prioritised planning is not complete in general: a fixed priority ordering can
...
is a reactive variant of prioritised planning that makes greedy decisions
...
A small number of works address this integration in the decentralised setting.
...
Communication constraints are a recognised challenge in multi-robot systems.
\citet{parker1998alliance} showed that fault-tolerant behaviour can emerge from
...
\citet{otte2020auctions} analyse auction-based allocation under bandwidth and
...
Modern warehouse systems are characterised by the \emph{induct--eject}
...
For the purposes of this thesis, the warehouse is modelled as a discrete grid
...
\begin{enumerate}
  ...
  \item \textbf{Persistently disconnected communication.}
        ...
        Existing decentralised methods (CBBA, GCBBA, DMCHBA) break down or
...
LCBA inherits the $\frac{1}{2}$-optimality guarantee and convergence properties
